...
import hu.bme.mit.mdsd.m2m2c.queries.Entity_C_SecondCaseMatcher
...
class EntityRule_C_SecondCase extends AbstractRule{
	...
	new(ViatraQueryEngine engine, IModelManipulations manipulation, TraceRoot traceRoot) {
		...
		rule = createRule.name("EntityRule")
			...
			.precondition(Entity_C_SecondCaseMatcher.querySpecification)
			.action(CRUDActivationStateEnum.CREATED) [
			println('''EntityRule CREATED V2 («entity.name»)''')
