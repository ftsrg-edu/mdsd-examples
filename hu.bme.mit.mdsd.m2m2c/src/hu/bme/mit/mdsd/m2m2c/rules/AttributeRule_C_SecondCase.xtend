...
import hu.bme.mit.mdsd.m2m2c.queries.Attribute_C_SecondCaseMatcher
...
class AttributeRule_C_SecondCase extends AbstractRule {
	...
	new(ViatraQueryEngine engine, IModelManipulations manipulation, TraceRoot traceRoot) {
		...
		rule = createRule.name("AttributeRule")
			...
			.precondition(Attribute_C_SecondCaseMatcher.querySpecification)
			.action(CRUDActivationStateEnum.CREATED) [
			println('''AttributeRule CREATED V2 («attribute.name»)''')
