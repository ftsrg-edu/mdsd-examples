...
public class RelationMatcher extends BaseMatcher<RelationMatch> {
   ...
   * 
   */
  public static RelationMatcher on(final ViatraQueryEngine engine) throws ViatraQueryException {
    // check if matcher already exists
    RelationMatcher matcher = engine.getExistingMatcher(querySpecification());
    if (matcher == null) {
    	matcher = (RelationMatcher)engine.getMatcher(querySpecification());
    }
    return matcher;
  }
   ...
   * 
   */
  public static RelationMatcher create() throws ViatraQueryException {
    return new RelationMatcher();
  }
  
  private final static int POSITION_R = 0;
  
  private final static Logger LOGGER = ViatraQueryLoggingUtil.getLogger(RelationMatcher.class);
   ...
   * 
   */
  private RelationMatcher() throws ViatraQueryException {
  ...
  protected RelationMatch tupleToMatch(final Tuple t) {
    try {
    	return RelationMatch.newMatch((Relation) t.get(POSITION_R));
  ...
  protected RelationMatch arrayToMatch(final Object[] match) {
    try {
    	return RelationMatch.newMatch((Relation) match[POSITION_R]);
    } catch(ClassCastException e) {
  ...
  protected RelationMatch arrayToMatchMutable(final Object[] match) {
    try {
    	return RelationMatch.newMutableMatch((Relation) match[POSITION_R]);
    } catch(ClassCastException e) {
    	LOGGER.error("Element(s) in array not properly typed!",e);
    	return null;
    }
   ...
   * 
   */
  public static IQuerySpecification<RelationMatcher> querySpecification() throws ViatraQueryException {
